...
# ====== 데이터셋/학습 ======
# 안정화: 환경변수로 DataLoader 워커 조절 (기본 0)
DATALOADER_WORKERS = int(os.getenv("DL_WORKERS", "0"))
...
def train_transformer_model(ticker, epochs=50):
    ...
    dataloader = DataLoader(
        ...
        batch_size=32,
        shuffle=True,
        num_workers=DATALOADER_WORKERS  # 안정성 기본 0, 필요 시 DL_WORKERS=2로 조정
    )
...
def compute_breadth_above_ma20(top_list):
    ...
    디버그 로그 포함: total/above/skipped 출력
    """
    if not top_list:
        print("[BREADTH] top_list 비어 있음 → 0.0")
        return 0.0
    count_above, total, skipped = 0, 0, 0
    for t in top_list:
        try:
            ...
            continue
    if total == 0:
        print(f"[BREADTH] 계산불가: total=0 (skipped={skipped}/{len(top_list)}) → 0.0")
        return 0.0
    ratio = count_above / total
    print(f"[BREADTH] MA20 위 {count_above}/{total} (skipped={skipped}) → {ratio*100:.1f}%")
    return ratio
...
def compute_top_n(current_top=None):
    ...
    n = max(20, min(60, n))
    print(f"[TOP-N] equity≈{equity:.0f}, regime={regime}, scale={scale:.2f}, n={n}")
    return n
...
if __name__ == "__main__":
    ...
    # 초기 설정 — 동적 N 적용 (인자 없이도 동작 안전)
    top_tickers = get_top_tickers()  # 내부에서 compute_top_n() 호출
    print(f"거래량 상위 코인(N={len(top_tickers)}): {top_tickers}")
    ...
    # ✅ 즉시 재학습 방지: 초기 학습 직후 최근 갱신시각을 현재로 세팅
    ...
    try:
        while True:
            ...
            if (now - last_top_update) >= timedelta(hours=6):
                top_tickers = get_top_tickers()  # 내부에서 compute_top_n() 호출
                print(f"[{now}] 상위 코인 업데이트(N={len(top_tickers)}): {top_tickers}")
                last_top_update = now
                for ticker in top_tickers:
                    model = models.get(ticker)
                    need_retrain = (model is None) or ((now - last_trained_time.get(ticker, datetime.min)) > TRAINING_INTERVAL)
                    if need_retrain:
                        model = train_transformer_model(ticker)
                        if model is None:
                            continue
            ...
            for ticker in target_tickers:
                ...
                try:
                    ...
                    if ticker in entry_prices:
                        ...
                        will_sell = False
                        try:
                            T_buy_dummy, T_sell_eff = compute_ml_threshold(ticker, regime)
