...
def rank_universe(candidates, surge_dict):
    """모든 후보(candidates)를 점수화해서 정렬. spread 좁을수록↑, 급등 가점."""
    scored = []
    for t in candidates:
        sp = get_spread_bp(t)
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            # bear라도 전면 차단하지 않음 (하드 블록은 DD/리스크만)
            ...
            # ==== 하드 게이트 제거: 모든 후보 중 랭킹순으로 슬롯만 배분 ====
            base_block_new = BUY_BLOCK_DD or risk_block  # 오직 DD/리스크만 하드 블록
            universe_for_new = list(target_tickers - held_tickers)
            ranked_all = rank_universe(universe_for_new, recent_surge_tickers)
            buy_allowed_tickers = set([] if (base_block_new or slots_available == 0) else ranked_all[:slots_available])
            ...
            for ticker in target_tickers:
                ...
                try:
                    ...
                    if not can_new_buy:
                        if ticker not in entry_prices:
                            log.info(f"[{ticker}] 신규 매수 스킵: 후보 아님/슬롯0/차단(risk/dd)/bear예외X")
                    else:
